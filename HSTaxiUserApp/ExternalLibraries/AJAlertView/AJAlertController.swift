...
class AJAlertController: UIViewController {
    ...
    private var strAlertTitle = Utility.shared.getLanguage()?.value(forKey: "appname")
    ...
    func changeToRTL() {
        self.btnOK.config(color: btnOtherColor, size: 17, align: .center, title: "ok")
        if UserModel.shared.getAppLanguage() == "Arabic" || UserModel.shared.getAppLanguage() == "Hebrew"{
    ...
    private func setupAJAlertController(){
        ...
        lblTitle.font = UIFont.init(name: APP_FONT_REGULAR, size: 17)
        lblAlertText?.text   = strAlertText
        lblAlertText?.font = UIFont.init(name: APP_FONT_REGULAR, size: 15)
        ...
        btnOK.titleLabel?.font = UIFont.init(name: APP_FONT_REGULAR, size: 15)
        btnOther.titleLabel?.font = UIFont.init(name: APP_FONT_REGULAR, size: 15)
        btnCancel.titleLabel?.font = UIFont.init(name: APP_FONT_REGULAR, size: 15)
    ...
    public func showAlert( aStrMessage:String,
                    ...
                    aOtherBtnTitle:String? ,status: String?,
                    completion : alertCompletionBlock){
        configure( message: Utility.shared.getLanguage()?.value(forKey:aStrMessage) as! String, btnCancelTitle: Utility.shared.getLanguage()?.value(forKey:aCancelBtnTitle!) as? String, btnOtherTitle: Utility.shared.getLanguage()?.value(forKey:aOtherBtnTitle!) as? String)
        show()
        if status != "1" {
            self.changeToRTL()
        }
    ...
    public func showAlertWithOkButton( aStrMessage:String, status: String?,
                                completion : alertCompletionBlock){
        configure(message: Utility.shared.getLanguage()?.value(forKey:aStrMessage) as? String ?? "", btnCancelTitle: nil, btnOtherTitle: nil)
        show()
        if status != "1" {
            self.changeToRTL()
        }
