...
-(void)joinToCall:(NSString *)room platform:(NSString *)platform call_type:(NSString *)type{
    ARDSettingsModel *settingsModel = [[ARDSettingsModel alloc] init];
    [settingsModel storeVideoResolutionSetting:@"640x480"];
    if ([type isEqualToString:@"audio"]) {
        NSLog(@"audio call off video");
    ...
    _client = [[ARDAppClient alloc] initWithDelegate:self];
    [_client connectToRoomWithId:room settings:settingsModel isLoopback:false];
}
- (void)speakerOff{
    [self speaker:FALSE];
}
...
-(void)joinToNextRoom:(NSString *)name platform:(NSString *)platform{
    ARDSettingsModel *settingsModel = [[ARDSettingsModel alloc] init];
    [settingsModel storeVideoResolutionSetting:@"640x480"];
    _videoCallView.platform = platform;
    _client = [[ARDAppClient alloc] initWithDelegate:self];
...
- (void)enlargeView:(NSString *)type{
    [_videoCallView updateViews];
