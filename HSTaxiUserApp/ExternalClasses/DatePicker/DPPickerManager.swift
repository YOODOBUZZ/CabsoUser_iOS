...
//        let image = UIImage(named: "ic_close")?.withRenderingMode(.alwaysTemplate)
        ...
        let image = #imageLiteral(resourceName: "close_icon").withRenderingMode(.alwaysTemplate)
        // trick
        ...
        close.isHidden = true
        // ok button
        let ok = UIAlertAction(title: Utility.shared.getLanguage()?.value(forKey: "ok") as? String, style: .default) { (action) in
            completion?(false)
        }
        alertView.addAction(ok)
        // cancel button
        let cancel = UIAlertAction(title: Utility.shared.getLanguage()?.value(forKey: "cancel") as? String, style: .cancel) { (action) in
            completion?(false)
        }
